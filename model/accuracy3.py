...
class PositivesDataset(Dataset):
    def __init__(self, dataframe: pd.DataFrame):
        ...
        assert len(self.task_samples) == len(self.translator_samples), "Mismatch between task and translator samples"
...
if __name__ == "__main__":
    wandb.init(project="translation-retrieval-sweep",
               config={
                   "batch_size": 128,
                   "learning_rate": 1e-4,
                   "latent_dim": 10,
    ...
    optimizer_translator = torch.optim.Adam(model_translator.parameters(), lr=cfg.learning_rate)

    # Training loop with eval and metrics
    for epoch in range(cfg.epochs):
        train_loss = train_step(
            train_loader, model_task, model_translator, loss_fn,
            miner, optimizer_task, optimizer_translator, device
        )
        val_loss = eval_step(
            val_loader, model_task, model_translator, loss_fn,
            miner, device
        )
        metrics = calculate_accuracy_metrics(
            val_loader, model_task, model_translator,
        ...
        print(f"Epoch {epoch+1}/{cfg.epochs} - train_loss: {train_loss:.4f} - val_loss: {val_loss:.4f} - mrr: {metrics['mrr']:.4f}")
